...
const HomePage = () => {
  ...
  const fetchUserData = useCallback(async () => {
    if (!storedUser.email) {
      console.error("⚠️ No email found in sessionStorage! Cannot fetch user data.");
      return;
    }
  ...
  const fetchHomepagePosts = useCallback(async () => {
    try {
      ...
      const response = await axios.get("/api/feed/homepage", {
        params: { email }
      });
  ...
  useEffect(() => {
    // Always fetch fresh data when navigating or reloading
      sessionStorage.removeItem("homepagePosts");
      fetchHomepagePosts();
            ...
            try {
              ...
              if (!postOwnerId) {
                ...
                try {
                  ...
                  const postResponse = await axios.get(`/api/posts/${post._id}`);
                  if (postResponse.data && postResponse.data.user_email) {
                    // Fetch the user to get their ID
                    const userResponse = await axios.get(`/api/users/email/${postResponse.data.user_email}`);
