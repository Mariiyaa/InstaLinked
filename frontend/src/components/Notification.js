...
const Notification = () => {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const response = await fetch(`/api/notifications/user/${user._id}`, {
                credentials: 'include',
                headers: {
    ...
    const markAllNotificationsAsRead = async () => {
        try {
            ...
            const response = await fetch(`/api/notifications/mark-all-read/${user._id}`, {
                method: 'PUT',
                credentials: 'include'
            });
    ...
    const handleFollowBack = async (senderId) => {
        ...
        try {
            const user = JSON.parse(sessionStorage.getItem('user'));
            const response = await fetch(`/api/follow/${senderId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const handleNotificationClick = async (notification) => {
        ...
        if (notification.type === 'follow' && notification.sender?._id) {
            ...
            try {
                const response = await axios.get(`/api/posts/${notification.postId}`);
                if (response.data) {
                    setSelectedPost({
                        ...response.data,
                        user: {
