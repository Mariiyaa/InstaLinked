    ...
    const fetchUserData = async () => {
      try {
        ...
        if (isLoggedInUser) {
          ...
          userResponse = await axios.get(`/api/user/data`, {
          ...
          try {
            ...
            userResponse = await axios.get(`/api/user/${userId}`);
          }
        }
        ...
        const postsResponse = await axios.get('/api/feed/homepage', {
          params: { email: userResponse.data.email }
        });

        // Get followers and following counts
        const [followersResponse, followingResponse] = await Promise.all([
          axios.get(`/api/users/${userResponse.data._id}/followers`),
          axios.get(`/api/users/${userResponse.data._id}/following`)
        ]);
        ...
        // If this is the logged-in user, update sessionStorage with fresh data
        if (isLoggedInUser) {
